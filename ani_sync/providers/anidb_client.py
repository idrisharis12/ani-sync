...
def pure_md4(message: bytes) -> bytes:
    ...
    def lrot(val, n): return ((val << n) & 0xFFFFFFFF) | (val >> (32 - n))

    def FF(a, b, c, d, k, s): return lrot((a + F(b, c, d) + X[k]) & 0xFFFFFFFF, s)
    def GG(a, b, c, d, k, s): return lrot((a + G(b, c, d) + X[k] + 0x5A827999) & 0xFFFFFFFF, s)
    def HH(a, b, c, d, k, s): return lrot((a + H(b, c, d) + X[k] + 0x6ED9EBA1) & 0xFFFFFFFF, s)

    length = len(message)
    msg = message + b'\x80' + b'\x00' * ((56 - (length + 1) % 64) % 64) + struct.pack('<Q', length * 8)
    ...
    for i in range(0, len(msg), 64):
        ...
        A = HH(A, B, C, D,  0,  3); D = HH(D, A, B, C,  8,  9); C = HH(C, D, A, B,  1, 11); B = HH(B, C, D, A,  9, 15)
        A = HH(A, B, C, D,  2,  3); D = HH(D, A, B, C, 10,  9); C = HH(C, D, A, B,  3, 11); B = HH(B, C, D, A, 11, 15)
        A = HH(A, B, C, D,  4,  3); D = HH(D, A, B, C, 12,  9); C = HH(C, D, A, B,  5, 11); B = HH(B, C, D, A, 13, 15)
        A = HH(A, B, C, D,  6,  3); D = HH(D, A, B, C, 14,  9); C = HH(C, D, A, B,  7, 11); B = HH(B, C, D, A, 15, 15)
